...
1) "dir"
2) "/usr/local/bin"  # 如果这个目录存在 dump.rdb，启动就会自动恢复其中的数据
```
...
如果这个 aof 文件存在错位，这时的 Redis 是启动不了的(会报下面的错误)，**需要修复这个 aof 文件**。
...
```bash
# 第1步：订阅一个频道
127.0.0.1:6379> subscribe nini      # 订阅一个频道，频道名是 nini
Reading messages... (press Ctrl-C to quit)
1) "subscribe"
2) "nini"
3) (integer) 1

# ....等待读取推送的信息：发送端向nini频道发送消息后，会自动同步到这个频道中，显示在下面...

# 第3步：频道自动接收消息（自动显示到下面）
1) "message"      # 消息
2) "nini"         # 频道
3) "hello,nini!"  # 消息内容
```
...
```bash
# 第2步：向频道发送消息
127.0.0.1:6379> publish nini "hello,nini!"   # 向频道nini发送消息
(integer) 1
```
...
79  <-  80  <- 81，上一个Master链接下一个Slaver，即：79是80的主机，80是81的主机。

不过查看80的库信息会发现其仍然为从机性质，不能手动写入数据。
...
```bash
# 在kconfig中创建哨兵配置文件
(base) hillking@fengwennideMacBook-Pro kconfig % vim sentinel.conf
(base) hillking@fengwennideMacBook-Pro kconfig % ls
redis.conf	redis79.conf	redis80.conf	redis81.conf	sentinel.conf

# 然后在这个文件中写入以下内容： sentinel monitor 被监控的名称 host port 1
# 其中：数字1代表主机挂了，slave投票让谁接替成为主机，票数最多的就会成为主机
sentinel monitor myredis 127.0.0.1 6379 1

# 哨兵配置文件中可以写入很多东西，这里只写了最简单的一条。其他信息百度自学
```

2. 命令行启动哨兵

```bash
(base) hillking@fengwennideMacBook-Pro bin % redis-sentinel kconfig/sentinel.conf
2206:X 06 May 2022 14:41:12.446 # oO0OoO0OoO0Oo Redis is starting oO0OoO0OoO0Oo
2206:X 06 May 2022 14:41:12.446 # Redis version=6.2.6, bits=64, commit=00000000, modified=0, pid=2206, just started
2206:X 06 May 2022 14:41:12.446 # Configuration loaded
2206:X 06 May 2022 14:41:12.447 * Increased maximum number of open files to 10032 (it was originally set to 256).
2206:X 06 May 2022 14:41:12.447 * monotonic clock: POSIX clock_gettime
                _._                                                  
           _.-``__ ''-._                                             
      _.-``    `.  `_.  ''-._           Redis 6.2.6 (00000000/0) 64 bit
  .-`` .-```.  ```\/    _.,_ ''-._                                  
 (    '      ,       .-`  | `,    )     Running in sentinel mode
 |`-._`-...-` __...-.``-._|'` _.-'|     Port: 26379    【哨兵端口号】
 |    `-._   `._    /     _.-'    |     PID: 2206      【哨兵进程pid】
  `-._    `-._  `-./  _.-'    _.-'                                   
 |`-._`-._    `-.__.-'    _.-'_.-'|                                  
 |    `-._`-._        _.-'_.-'    |           https://redis.io       
  `-._    `-._`-.__.-'_.-'    _.-'                                   
...
2206:X 06 May 2022 14:41:12.449 # Sentinel ID is 07d4f18d904b9a301d89f2182eee08f2c0ee6ac6
2206:X 06 May 2022 14:41:12.449 # +monitor master myredis 127.0.0.1 6379 quorum 1 【监视的机器】
2206:X 06 May 2022 14:41:12.449 * +slave slave 127.0.0.1:6380 127.0.0.1 6380 @ myredis 127.0.0.1 6379  【发现监视的主机有两个从机80和81】
2206:X 06 May 2022 14:41:12.450 * +slave slave 127.0.0.1:6381 127.0.0.1 6381 @ myredis 127.0.0.1 6379


```

3. 此时关闭主机6379，就会从从机中随机选择一个服务器作为新的主服务器。（有一个投票算法）。等待一会，会发现80、81其中的一个变成了主机。证明哨兵进程是有效的。
...
role:master
connected_slaves:1
slave0:ip=127.0.0.1,port=6380,state=online,offset=34859,lag=0
......
...
```bash
fileover：故障转移
sdown：主观宕机    odown：客观宕机
```

![img](img/%E6%88%AA%E5%B1%8F2022-05-06%2014.51.58.png)

5. 此时如果主节点79回来了，只能作为新主节点的从节点。

```bash
# 重新启动79节点，查看库信息，发现其变成了从库
(base) hillking@fengwennideMacBook-Pro bin % redis-server kconfig/redis79.conf
(base) hillking@fengwennideMacBook-Pro bin % redis-cli -p 6379
127.0.0.1:6379> info replication
# Replication
...
==============================================================
# 哨兵进程窗口中，出现了convert，表示79转移成了81的从库
2206:X 06 May 2022 15:07:12.465 * +convert-to-slave slave 127.0.0.1:6379 127.0.0.1 6379 @ myredis 127.0.0.1 6381 
...
role:master
connected_slaves:2
slave0:ip=127.0.0.1,port=6380,state=online,offset=171718,lag=0
slave1:ip=127.0.0.1,port=6379,state=online,offset=171718,lag=0
......
```
